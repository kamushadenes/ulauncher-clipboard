...
def showStatus(status):
    return RenderResultListAction(
        [
            ExtensionResultItem(
                name=status, on_enter=DoNothingAction(), highlightable=False
            )
        ]
    )
...
class KeywordQueryEventListener(EventListener):
    def on_event(self, event, extension):
        ...
        if not ensureStatus(manager):
            return showStatus(
                "Could not start {}. Please make sure you have it on your system and that it is not disabled.".format(
                    manager.name
                )
            )
